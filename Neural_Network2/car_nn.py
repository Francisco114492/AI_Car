...
#TRACK_IMAGE_PATH="FIA/Track1.png"
#SCREEN_WIDTH1, SCREEN_HEIGHT1 = 1400, 800    # Track1
#START_X1, START_Y1 = 100, 500                # Track1
#START_ANGLE1 = 0                            # Track1

#TRACK_IMAGE_PATH="FIA/Track2.png"
#SCREEN_WIDTH2, SCREEN_HEIGHT2 = 1400, 800    # Track2
#START_X2, START_Y2 = 120, 100                # Track2
#START_ANGLE2= math.pi/2                     # Track2

#TRACK_IMAGE_PATH="Track3.png"
#SCREEN_WIDTH, SCREEN_HEIGHT = 900, 500     # Track3
#START_X, START_Y = 645, 445                # Track3
#START_ANGLE = -3*math.pi/4                 # Track3

#TRACK_IMAGE_PATH="FIA/Track4.png"
#SCREEN_WIDTH, SCREEN_HEIGHT = 900, 726     # Track4
#START_X, START_Y = 312, 692                # Track4
#START_ANGLE = -math.pi/2                   # Track4

# ---------------------------------------------- #
# Loading path for the neural network navigation #
# ---------------------------------------------- #

def load_path(path_file="track_path.txt"):
    path_points = []
...
def load_checkpoints(path="track_checkpoints.txt"):
    checkpoints = []
    with open(path) as f:
        next(f)  # skip header
        for line in f:
...
class Car:
    def __init__(self, x, y, angle, neural_net):
        ...
        self.output = []
        self.last_start_dist = float('inf')
        self.lap_count = 0
        self.lap_timer = 0
        self.current_sector = 0
        self.sector_times = []
        self.sector_timer = 0
        self.valid_lap = True  # to ensure all checkpoints were hit
    ...
    def update(self, track, dt,sim_time, sensors, sensors_before, START_X, START_Y, surface=None):
        '''
        ...
        nav_angle, nav_distance = self.get_navigation_data(TRACK_PATH)

        inputs = np.array(sensors + [nav_angle, nav_distance]).reshape(-1, 1)
        if surface:
            self.network.draw(surface, inputs)
        sector_input = self.current_sector / len(CHECKPOINTS)  # normalized [0, 1]
        #inputs = np.array(sensors + [nav_angle, nav_distance, sector_input]).reshape(-1, 1)
        ...
        start_dist = math.hypot(self.x - START_X, self.y - START_Y)

        if start_dist < 20 and self.last_start_dist >= 20:
            if self.current_sector == len(CHECKPOINTS):  # completed all
                self.lap_count += 1
                self.valid_lap = True
            else:
                self.valid_lap = False  # missed some
        ...
        self.last_start_dist = start_dist

        if self.current_sector < len(CHECKPOINTS):
            cx, cy = CHECKPOINTS[self.current_sector]
            checkpoint_dist = math.hypot(self.x - cx, self.y - cy)
            if checkpoint_dist < 20:
                self.sector_times.append(self.sector_timer)
                self.sector_timer = 0
    ...
    def draw(self, screen, track):
        ...
        if self.alive:  # only first car draws static elements
            # Draw path points
            for (x, y) in TRACK_PATH[::10]:  # skip some for performance
                pygame.draw.circle(screen, (0, 0, 255), (int(x), int(y)), 1)  # blue dots

            pygame.draw.circle(screen, (255, 165, 0), (int(CHECKPOINTS[self.current_sector-1][0]), int(CHECKPOINTS[self.current_sector-1][1])), 5)  # orange circles


    def get_navigation_data(self, track_path):
        ...
        # Predict the "target" point slightly ahead of current index
        lookahead = 2
        target_index = min(closest_index + lookahead, len(track_path) - 1)
        target_x, target_y = track_path[target_index]
        ...
        distance = math.hypot(dx, dy) / 100.0  # scale distance

        return angle_diff / math.pi, distance  # normalize to [-1, 1] and scale
        #return distance
...
def handle_events(cars, running, sim_duration, to_save, track_n, screen):
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN and event.key == pygame.K_s:
            ...
            if to_save==True:
                ...
                to_save=True
    return cars, running, sim_duration, to_save
...
def calc_fitness(cars, track, cache_fitness, sim_time, best_distance):
    ...
    for car in cars:
        fitness = cache_fitness + car.current_sector*100
        fitness -= 300 if car.check_collision(track) else 0
...
def main(track_n, screen):
    ...
    global TRACK_PATH, CHECKPOINTS
    TRACK_PATH = load_path()
    CHECKPOINTS = load_checkpoints()
    # Genetic Algorithm Parameters
    generation = 0
    cars = []

    input_size = len(SENSOR_ANGLES) + 2  # 7 sensors + 2 nav inputs # <---
    ...
    while running:
        ...
        while sim_time < sim_duration and any(car.alive for car in cars):
            ...
            sim_time += dt
            total_steps+=1
            cars, running, sim_duration, to_save=handle_events(cars, running, sim_duration, to_save, track_n, screen)
            ...
            for car in cars:
                if car.alive:
                    ...
                    if car.lap_count < 2:
                        ...
                        nav_angle, nav_distance = car.get_navigation_data(TRACK_PATH)
                        #print(nav_distance)
                        if nav_distance < 0.1:
                            cache_fitness += 0.1
                        if nav_angle < 0.2:
                        ...
                        if car == best_car_dist:
                            car.update(track, dt,sim_time, sensors, sensors_before, START_X, START_Y, nn_surface)
                        else:
                            car.update(track, dt,sim_time, sensors, sensors_before, START_X, START_Y)
                    car.draw(map, track)
            ...
            nn_surface = pygame.transform.scale(nn_surface, (400, 400))
    ...
    os.makedirs("Neural_Network2/"+txt_path, exist_ok=True)
    write_file("Neural_Network2/"+txt_path+"/deaths.txt",0,deaths_arr) # save the crash positions of all the cars
    write_file("Neural_Network2/"+txt_path+"/best_behaviour.txt",1,best_car_arr) # save the best behaviour of the car that did the most distance in all the generations
    write_file("Neural_Network2/"+txt_path+"/save_behaviour.txt",1,save_arr) # save the best behaviour of the car that did the most distance is the generations chosen by the user (by hitting s)
    write_file("Neural_Network2/"+txt_path+"/fitness.txt",2,fitness_arr) # save the fitness of the best car for each generations
    return
